...
const replacement = `  const verifyPassword = (password: string, storedHash: string, username?: string): boolean => {
...
// Find everything from "const verifyPassword = " up to just before "const generateToken ="
const regex = /const verifyPassword = [\s\S]*?(?=const generateToken =)/;
const newCode = code.replace(regex, replacement + '\n\n  ');

fs.writeFileSync('server.ts', newCode);
console.log("Fixed verifyPassword");
